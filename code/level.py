...
from sprites import Ordinary,waterSprite,natFlower,Tree
...
class Level:
    ...
    def setup(self):
        ...
        for tree in mapData.get_layer_by_name("Trees"):
            # Dont need to multiply as these are not tiles and therefore have pixel measurements
            Tree((tree.x, tree.y),tree.image,[self.allSprites,self.collisionSprites], tree.name)
        ...
        for playerItems in mapData.get_layer_by_name("Player"):
            if playerItems.name == "Start":
                self.player = Player((playerItems.x,playerItems.y), self.allSprites, self.collisionSprites)
